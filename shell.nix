let
  ...
  inherit (pkgs) lib;
  sdlPkgs = pkgs.recurseIntoAttrs (pkgs.callPackage ./nix { });
  sdlDrvs = lib.filterAttrs (_: value: lib.isDerivation value) sdlPkgs;
  ...
  filterDrvs = inputs:
    lib.filter
      (drv:
        ...
        drv.pname == null ||
        !(lib.any (name: name == drv.pname || name == drv.name) (lib.attrNames sdlDrvs)))
      inputs;
...
in
  ...
  (mkShell {
    inputsFrom = lib.attrValues sdlDrvs;
